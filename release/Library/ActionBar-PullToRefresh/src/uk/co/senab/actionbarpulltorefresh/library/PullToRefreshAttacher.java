...
public class PullToRefreshAttacher implements View.OnTouchListener {
	...
	private boolean mIsBeingDragged, mIsRefreshing, mHandlingTouchEventFromDown,mHandlingTouchEventFromMove,mTouchEventDownVirtual;
	...
	protected PullToRefreshAttacher(Activity activity, Options options) {
		...
		mRefreshScrollDistance = this.getScreenHeight(activity) * DEFAULT_REFRESH_SCROLL_DISTANCE;
	...
	void addRefreshableView(View view, ViewDelegate viewDelegate,
		...
		if(DEBUG)Log.d(LOG_TAG,view.toString());
	...
	final boolean onInterceptTouchEvent(View view, MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE: {
            	if (mHandlingTouchEventFromMove && mTouchEventDownVirtual){
            		 if (canRefresh(true, params.onRefreshListener)
                         ...
                         if (DEBUG) Log.d(LOG_TAG, "setting initialMotionY: "+mInitialMotionY);
                         mTouchEventDownVirtual = false;
                     }
            	}
	...
	final boolean onTouchEvent(View view, MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE: {
            	if (!mHandlingTouchEventFromMove){
            		mHandlingTouchEventFromDown = true;
            		mHandlingTouchEventFromMove = true;
            		mTouchEventDownVirtual = true;
            	}
            	else {
            		mTouchEventDownVirtual = false;
            	}
	...
	void resetTouch() {
		...
		mHandlingTouchEventFromMove = false;
		mTouchEventDownVirtual = false;
	...
	private int getScreenHeight(Context context){
		...
		int height = 0;
		/*
		 * New api method since Honeycomb MR2
		 */
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.HONEYCOMB_MR2){
			...
			height = display.getWidth();  // for compatibility with api < 13
		}
		return height;
	}
