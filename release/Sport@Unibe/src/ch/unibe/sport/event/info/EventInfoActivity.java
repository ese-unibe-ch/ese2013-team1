...
/**
 * Activity that handles an event's information
 * (rating, calendar, main info card, ...)
 * @author Team 1
 *
 */
public class EventInfoActivity extends ProxySherlockFragmentActivity {
	...
	public static void show(final Context context,final String hash) {
		if (hash == null || hash.length() == 0 || context == null) return;
		int eventID = new Events(context).getEventID(hash);
		show(context,eventID);
	}
