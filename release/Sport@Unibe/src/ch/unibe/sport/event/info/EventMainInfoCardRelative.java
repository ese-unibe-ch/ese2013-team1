...
public class EventMainInfoCardRelative extends RelativeLayout implements INodable {
	...
	public void setEvent(Event event){
		...
		Date dateAttended = event.getAttendedDate();
		if (days.length() > 0){
		...
		if (dateAttended != null){
			viewHolder.dayAttended.setText("will attend "+dateAttended.print()+", ");
			viewHolder.dayAttended.setVisibility(VISIBLE);
		}
		else {
			viewHolder.dayAttended.setVisibility(GONE);
		}
		if (viewHolder.day.getVisibility() == GONE && viewHolder.dayAttended.getVisibility() == GONE){
		...
		if (event.getPlace() != null){
			viewHolder.place.setText(event.getPlace().getPlaceName());
	...
	/**
	 * OnCLickListener implementation, that defines action after user
	 * clicks "Show on map"
	 * 
	 * @author Team 1 2013
	 */
	...
	/**
	 * Controls the favorite course star button
	 * @author Team 1
	 *
	 */
